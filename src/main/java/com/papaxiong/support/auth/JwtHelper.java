...
public class JwtHelper {
     ...
     * @param userId - 用户编号
     * @param userName - 用户名
     * @param  - 客户端信息（变长参数），目前包含浏览器信息，用于客户端拦截器校验，防止跨域非法访问
     * @Data: 2018/7/28 19:26
     * @Modified By:
     */
    public static String generateJWT(String userId, String userName, String jwtKey, Map<String, Object> paramMap ) {
        //签名算法，选择SHA-256
        ...
        JwtBuilder builder = Jwts.builder().setHeader(headMap)
                 ...
                 */
                //加密后的客户编号
                .claim("userId", AESSecretUtil.encryptToStr(userId, jwtKey))
                //客户名称
...
//                .claim("userAgent", identities[0])
     ...
     * @Modified By:
     */
    public static String validateLogin(String jsonWebToken, String jwtKey) {
        Map<String, Object> retMap = null;
        try {
            Claims claims = parseJWT(jsonWebToken);
            if (claims != null) {
                //解密客户编号
                String decryptUserId = AESSecretUtil.decryptToStr((String)claims.get("userId"), jwtKey);
                retMap = new HashMap<>();
                //加密后的客户编号
                retMap.put("userId", decryptUserId);
                //客户名称
                retMap.put("userName", claims.get("userName"));
                retMap.put("userCode",claims.get("userCode"));
                //客户端浏览器信息
    //            retMap.put("userAgent", claims.get("userAgent"));
                //刷新JWT
                //retMap.put("freshToken", generateJWT(decryptUserId, (String)claims.get("userName"),SecretConstant.DATAKEY_PRISONER,(String)claims.get("domainName")));
            }else {
                logger.warn("[JWTHelper]-JWT解析出claims为空");
            }
        } catch (Exception e) {
            logger.error("jwt解析异常！exception={}", ExceptionUtils.getStackTrace(e));
        }
        return retMap!=null?JSONObject.toJSONString(retMap):null;
    }

    public static void main(String[] args) {
//        String jsonWebKey = generateJWT("123", "Judy", SecretConstant.DATAKEY_PRISONER,
//                "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/68.0.3440.106 Safari/537.36");
//        System.out.println(jsonWebKey);
//        Claims claims =  parseJWT(jsonWebKey);
//        System.out.println(claims);
//        System.out.println(validateLogin(jsonWebKey,SecretConstant.DATAKEY_PRISONER));
